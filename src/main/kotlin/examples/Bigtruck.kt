...
fun bigtruck(): String {
    ...
    ig.dijkstra(1)
    if (ig.doubleDistanceTo(n) == Double.POSITIVE_INFINITY) {
        return "impossible"
    }
    val nrOfItems = ((ceil(ig.doubleDistanceTo(n)) - ig.doubleDistanceTo(n)) / ITEM_BOOST).roundToInt() + items[1]
    return "${ig.doubleDistanceTo(n).roundToInt()} $nrOfItems"
}